...
## 7. 精华过滤与防污染（第三遍，2026-09-05）

**Anti-屎山 / 防数据污染评分器**：`pefmod_quality.py`

评分维度：等级(S/A/B/C) + 机制词M + 完整度 + 草稿特征 + 幻觉词密度 → 四档分级。

**1200 份 PEFMOD 文档实测结果**：
...
| B 可留 | 233 | 参考价值 |
| C 待清理 | 91 | 人工复核 |
| D 疑似垃圾 | 54 | 隔离（残篇<300字/草稿） |
| 重复对 | 48 | 清理冗余 |

**重复检测**：核心句内容指纹哈希碰撞 → 48 对真重复（迭代/bak/异格式/副本）。

**产出文件**：
- `pefmod_quality.json` — 1200 份四档分级明细
- `pefmod_dups.json` — 48 对重复清单（保留/归档建议）
- `pefmod_quality_summary.json` — 汇总
