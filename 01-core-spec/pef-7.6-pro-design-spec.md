...
SBX2是记忆层的物理载体，使用轻量级本地数据库（SQLite）配合哈希链校验。它不是简单的数据库，而是时间序列上的物理证据库——只增不减，任何历史记录都不可删除，只能追加新版本，形成完整的证据链。  [P0115]
...
深地层是记忆层的物理载体在时间序列上的延伸，对应SBX2黑匣子的底层历史存储。它记录所有历史版本的π标记绑定、原文坐标、验证日志。灰色地带是物理与逻辑的交界区，指那些物理状态不明确（如高阻态、未定义电平）、逻辑判定边界模糊（如大约、左右）的区域。深地层与灰色地带的接口是PEF 7.6 Pro 区别于7.6轻量模式的核心能力。  [P0139]
